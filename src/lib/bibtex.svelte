<script lang="ts">
	...
	function formatBibtex(p: Paper, v: Venue) {
		...
		// const auths = p.authors.map((a) => `${a.lastName}, ${a.firstName}`).join(' AND ');
		const auths = p.authors.map((a) => (typeof a === 'string' ? a : a.displayName)).join(' AND ');
		const type =
			v.venueType == 'journal'
				? 'article'
				: v.venueType == 'book'
